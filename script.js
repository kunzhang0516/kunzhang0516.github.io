...
var color = d3.scale.ordinal()
					...
					.domain(["Africa | North & East", "Africa | South & West", "America | North & Central"]);
							 
//Set the new x axis range
var xScale = d3.scale.log()
	.range([0, width])
	.domain([10,150]); //I prefer this exact scale over the true range and then using "nice"
...
circleGroup.selectAll("cars")
	.data(cars) //Sort so the biggest circles are below
	.enter().append("circle")
		.attr("class", function(d,i) { return "cars "+d.Ind;})
...
var voronoi = d3.geom.voronoi()
	.x(function(d) { return xScale(d.AverageCityMPG); })
	.y(function(d) { return yScale(d.AverageHighwayMPG); })
...
voronoiGroup.selectAll("path")
	.data(voronoi(cars)) //Use vononoi() with your dataset inside
	.enter().append("path")
	.attr("d", function(d, i) { return "M" + d.join("L") + "Z"; })
	.datum(function(d, i) { return d.point; })
	//Give each cell a unique class where the unique part corresponds to the circle classes
	.attr("class", function(d,i) { return "voronoi" + d.Ind; })
...
function showTooltip(d) {
	...
	var element = d3.select(".cars."+d.Ind);
	...
	$(element).popover({
		...
		content: function() { //the html content to show inside the tooltip
			return "<span style='font-size: 11px; text-align: center;'>" + d.Make + "</span>"; }
	});
